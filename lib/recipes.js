...
export async function getRecipesByCuisine(cuisineName) {
  const files = await getAllRecipeFiles();
  const recipes = await Promise.all(files.map(f => readRecipeFile(f)));
  return recipes.filter(r => r.cuisine.toLowerCase() === cuisineName.toLowerCase());
}